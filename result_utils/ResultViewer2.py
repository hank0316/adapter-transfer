...
class ResultViewer:
    ...
    def transferability(self, task1, task2, two_way=True):  # if two_way == False, return task1 -> task2
    ...
    def spearmanr(self, two_way=True):
        AB, AC, BC, ABC = [], [], [] ,[]
        ...
        for task_chain, target_performance in long_chain.items():
            ...
            AB.append(self.transferability(A, B, two_way))
            AC.append(self.transferability(A, C, two_way))
            BC.append(self.transferability(B, C, two_way))
            ABC.append(self.perf_gain[task_chain])  # performance gain of C task in ABC chain
        sr = stats.spearmanr
        return {'AB_ABC': sr(AB, ABC), 'AC_ABC': sr(AC, ABC), 'BC_ABC': sr(BC, ABC)}

    def spearmanr4(self, two_way=True):
        AD, BD, CD, ABCD = [], [], [] ,[]
        long_chain = self._find_matching_results(pattern='.*', chain_length=4)  
        for task_chain, target_performance in long_chain.items():
            A, B, C, D = task_chain.split('-')
            AD.append(self.transferability(A, D, two_way))
            BD.append(self.transferability(B, D, two_way))
            CD.append(self.transferability(C, D, two_way))
            ABCD.append(self.perf_gain[task_chain])  # performance gain of C task in ABC chain
        sr = stats.spearmanr
        return {'AD_ABCD': sr(AD, ABCD), 'BD_ABCD': sr(BD, ABCD), 'CD_ABCD': sr(CD, ABCD)}
